split_method_train = "Window"
split_method_test = "Window"
...
for mask_method in ["Random", "Purpose"]:
    ...
    for split_method_train in ["Random", "RandomFull", "Cascade", "CascadeFull", "Window", "MixBasic", "MixSuper", "Full"]:
        for split_method_test in ["Random", "RandomFull", "Cascade", "CascadeFull", "Window", "MixBasic", "MixSuper", "Full"]:
            if split_method_test == "Full":
                batch_size = 2
            else:
                batch_size = 4096
            
            s_toml = f'''logging_filepath = "logs/evaluate/MAEGIN_{split_method_train}_{mask_method}_on_{split_method_test}_{mask_method}.log" # Logging file path where logs will be saved, default to None, which may save to a default path that is determined by the Younger.
...
mask_ratio = {mask_ratio} # Ratio of elements (e.g., input tokens) to mask during training.
mask_method = "{mask_method}" # Strategy for masking elements: 'Random' for uniform masking, 'Purpose' for task-specific or guided masking.

# Nested Fields 0
[trainer]
checkpoint_savepath = "checkpoints/MAEGIN_{split_method_train}_{mask_method}" # Directory path to save checkpoint.
checkpoint_basename = "MAEGIN_{split_method_train}_{mask_method}" # Base name of the checkpoint for save/load.
...
# Nested Fields 1
[evaluator]
checkpoint_filepath = "checkpoints/MAEGIN_{split_method_train}_{mask_method}" # Path to load checkpoint.
...
# Nested Fields 4
[train_dataset]
meta_filepath = "data/{split_method_train}/training/meta.json" # Path to the metadata file that describes the dataset.
raw_dirpath = "data/{split_method_train}/training/items" # Directory containing raw input data files.
processed_dirpath = "data/{split_method_train}/training/processed" # Directory where processed dataset should be stored.
worker_number = 4 # Number of workers for parallel data loading or processing.

# Nested Fields 5
[valid_dataset]
meta_filepath = "data/{split_method_train}/validation/meta.json" # Path to the metadata file that describes the dataset.
raw_dirpath = "data/{split_method_train}/validation/items" # Directory containing raw input data files.
processed_dirpath = "data/{split_method_train}/validation/processed" # Directory where processed dataset should be stored.
worker_number = 4 # Number of workers for parallel data loading or processing.

# Nested Fields 6
[test_dataset]
meta_filepath = "data/{split_method_test}/test/meta.json" # Path to the metadata file that describes the dataset.
raw_dirpath = "data/{split_method_test}/test/items" # Directory containing raw input data files.
processed_dirpath = "data/{split_method_test}/test/processed" # Directory where processed dataset should be stored.
worker_number = 4 # Number of workers for parallel data loading or processing.
...
'''
            with open(f"configs/evaluate/MAEGIN_{split_method_train}_{mask_method}_on_{split_method_test}_{mask_method}.toml", "w") as f:
                f.write(s_toml)