for number_of_levels in [0, 1, 2, 3, 4, 5]:
    for generation_steps in [1, 2, 3, 4, 5, 6]:
        ...
        scheduled_sampling_level = list(range(0, generation_steps))
        
        batch_size = 512

        if number_of_levels == 0:
            checkpoint_filepath = f"checkpoints/MAEGIN_MixSuper_Purpose"
        else:
            checkpoint_filepath = f"checkpoints/MAEGIN_MixSuper_Purpose_S{number_of_levels}/MAEGIN_MixSuper_Purpose_best.cp"       

        s_toml = f'''
logging_filepath = "logs/evaluate_S/MAEGIN_MixSuper_Purpose_S{number_of_levels}_on_G{generation_steps}.log" # Logging file path where logs will be saved, default to None, which may save to a default path that is determined by the Younger.
...
# Nested Fields 0
[trainer]
checkpoint_savepath = "{checkpoint_filepath}" # Directory path to save checkpoint.
checkpoint_basename = "MAEGIN_MixSuper_Purpose" # Base name of the checkpoint for save/load.
checkpoint_keepdisk = 5 # Number of checkpoints to keep on disk.
# resume_filepath = "checkpoints/MAEGIN_MixSuper_Purpose" # Path to load checkpoint. If None, train from scratch.
...
# Nested Fields 1
[evaluator]
checkpoint_filepath = "{checkpoint_filepath}" # Path to load checkpoint.
...
# Nested Fields 4
[train_dataset]
meta_filepath = "data/MixSuper/training/meta.json" # Path to the metadata file that describes the dataset.
raw_dirpath = "data/MixSuper/training/items" # Directory containing raw input data files.
processed_dirpath = "data/MixSuper/training/processed" # Directory where processed dataset should be stored.
worker_number = 4 # Number of workers for parallel data loading or processing.

# Nested Fields 5
[valid_dataset]
meta_filepath = "data/MixSuper/validation/meta.json" # Path to the metadata file that describes the dataset.
raw_dirpath = "data/MixSuper/validation/items" # Directory containing raw input data files.
processed_dirpath = "data/MixSuper/validation/processed" # Directory where processed dataset should be stored.
worker_number = 4 # Number of workers for parallel data loading or processing.

# Nested Fields 6
[test_dataset]
meta_filepath = "data/Window/test/meta.json" # Path to the metadata file that describes the dataset.
raw_dirpath = "data/Window/test/items" # Directory containing raw input data files.
processed_dirpath = "data/Window/test/processed" # Directory where processed dataset should be stored.
worker_number = 4 # Number of workers for parallel data loading or processing.
...
'''
        with open(f"configs/evaluate_S/MAEGIN_MixSuper_Purpose_S{number_of_levels}_on_G{generation_steps}.toml", "w") as f:
            f.write(s_toml)